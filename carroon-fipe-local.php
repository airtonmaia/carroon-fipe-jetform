...
final class Carroon_Fipe_DB {
    ...
    public function enqueue_js() {
        ...
        // Define a configuração base dos campos
        $fields_config = [
            'tipo'           => 'tipo_veiculo',
            'marca'          => '__marca',
        ...
        // DEBUG: Verifica se estamos na página de edição e busca os dados do postmeta.
        // Isso injeta os dados diretamente no JS, evitando problemas de timing com o JetEngine.
        if ( is_page('editar-produto') && isset($_GET['pid']) ) {
            $post_id = absint($_GET['pid']);
            if ( $post_id > 0 && get_post($post_id) ) {
                // Busca os dados salvos usando os nomes dos campos definidos acima
                $saved_data = [
        ...
        // Passa a configuração e os dados salvos para o JavaScript.
        wp_localize_script('carroon-fipe-localdb','CARROON_FIPE_LOCAL',[
            'base'   => rest_url('carroon-fipe/v1'),
            'fields' => $fields_config,
            'saved'  => $saved_data, // Chave 'saved' conterá os dados do post ou um array vazio.
        ]);
    }

    /** ---------- Segurança da Página de Edição ---------- */
    public function check_edit_page_permissions(){
        // Alvo: apenas a página de edição, ajuste o slug se for diferente
        ...
        if ( ! $pid ) return; // Se não houver PID, não há o que checar
        ...
        // Permite o autor do post ou administradores
